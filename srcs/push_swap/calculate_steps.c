...
{
	if (min_stack(data->b) <= data->b->size / 2)
	{
		if (i <= data->a->size / 2)
		{
			if (min_stack(data->b) <= i)
				data->steps[i] = i;
			else
				data->steps[i] = min_stack(data->b);
		}
		else
			data->steps[i] += min_stack(data->b);
	}
	else if (min_stack(data->b) > data->b->size / 2)
		data->steps[i] += data->b->size - min_stack(data->b);
	data->steps[i]++;
}
